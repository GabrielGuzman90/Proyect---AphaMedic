...
class MedicamentoController extends Controller
{
    ...
    public function verCategoria(Request $request, $categoria)
    {
        ...
        $medicamentos = $this->obtenerMedicamentosFirebase()
            ->filter(function ($med) use ($busqueda) {

                return (
                    str_contains(strtolower($med->categoria), $busqueda) ||
                    str_contains(strtolower($med->nombre), $busqueda)
                )
                && $med->disponibilidad > 0;
            });

        // 🔽 filtros extra (los de tu vista)
        ...
        return view('medicamentos.categoria', [
            ...
            'categoria' => $categoria, // 👈 esto muestra lo que buscó
            'mgs' => $mgs,
